...
def evaluate(**kwargs):
    ...
    if eval_rendering:
        ...
        split_n_pixels = conf.get_int('train.split_n_pixels', 100)
    ...
    if False:
        with torch.no_grad():
            ...
            mesh_folder = '{}/{}'.format(evaldir, epoch)
            utils.mkdir_ifnotexists(mesh_folder)
            mesh_clean.export('{}/scan{}.ply'.format(mesh_folder, scan_id), 'ply')

    if eval_rendering:
        images_dir = '{}/rendering_{}'.format(evaldir, epoch)
        ...
        print("RENDERING EVALUATION {2}: psnr mean = {0} ; psnr std = {1}".format(
            "%.2f" % psnrs.mean(), "%.2f" % psnrs.std(), scan_id))
        psnrs = np.concatenate([psnrs, psnrs.mean()[None], psnrs.std()[None]])
        pd.DataFrame(psnrs).to_csv('{}/psnr_{}.csv'.format(evaldir, epoch))
